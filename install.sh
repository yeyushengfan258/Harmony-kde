...
install() {
  ...
  [[ -d ${SCHEMES_DIR}/Harmony.colors ]] && rm -r ${SCHEMES_DIR}/Harmony.colors
  ...
  [[ ${color} == '' ]] && \
  cp -ur ${SRC_DIR}/color-schemes/Harmony.colors                                    ${SCHEMES_DIR} && \
  cp -ur ${SRC_DIR}/color-schemes/Harmony.colors                                    ${PLASMA_DIR}/${name}/colors

  [[ ${color} == '-light' ]] && \
  cp -ur ${SRC_DIR}/color-schemes/Harmony.colors                               ${SCHEMES_DIR}
}

echo "Installing 'Harmony kde themes'..."
